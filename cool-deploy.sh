  ...
  case $1 in
    ...
    msg)
      ...
      if [ -n "$2" ]; then
        ...
        echo -e "\033[1;31mError:\033[0m No commit message provided after 'msg' command."
        exit 1
      fi
      shift 2
      ;;
...
check_env_coolify() {
  if [ -f ".env.coolify" ]; then
    echo -e "\033[1;31mError:\033[0m .env.coolify already exists! \033[1;33mCannot run init command.\033[0m"
    exit 1
  fi
}
...
load_env_coolify() {
  if [ ! -f ".env.coolify" ]; then
    echo -e "\033[1;31mError:\033[0m .env.coolify does not exist. \033[1;33mPlease run init command first.\033[0m"
    exit 1
  fi
  source .env.coolify
}
